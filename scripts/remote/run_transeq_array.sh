...
echo "transeq -trim -frame=6 -sformat1 pearson -osformat2 pearson $INPUT $OUTPUT"         >> $LOGS/transeq/${JOB_ID}.${SGE_TASK_ID}.all 2>&1
transeq -trim -frame=6 -sformat1 pearson -osformat2 pearson $INPUT $OUTPUT                >> $LOGS/transeq/${JOB_ID}.${SGE_TASK_ID}.all 2>&1
if[ -d $SPLITOUTPATH ]{
	SPLITOUTPUT=${SPLITOUTPATH}/${OUTBASENAME}${SGE_TASK_ID}.fa   >> $LOGS/transeq/${JOB_ID}.${SGE_TASK_ID}.all 2>&1
	echo "perl ${SCRIPTS}split_orf_on_stops.pl -i $OUTPUT -o $SPLITOUTPUT -l $FILTERLENGTH"  >> $LOGS/transeq/${JOB_ID}.${SGE_TASK_ID}.all 2>&1
	perl ${SCRIPTS}split_orf_on_stops.pl -i $OUTPUT -o $SPLITOUTPUT -l $FILTERLENGTH         >> $LOGS/transeq/${JOB_ID}.${SGE_TASK_ID}.all 2>&1
	date                                                          >> $LOGS/transeq/${JOB_ID}.${SGE_TASK_ID}.all 2>&1
